...
class ngsimDataset(Dataset):
    ...
    def collate_fn(self, samples):
        augmented_samples = []
        for hist, fut, nbrs, lat_enc, lon_enc in samples:
            augmented_samples.append((hist, fut, nbrs, lat_enc, lon_enc))
            
            for i in range(len(nbrs)):
                if nbrs[i].size > 0:
                    refPos_nbr = nbrs[i][-1, :] 
                    
                    new_hist = nbrs[i] - refPos_nbr
                    new_fut = fut - refPos_nbr if fut.size > 0 else fut
                    
                    new_nbrs = []
                    for n in nbrs:
                        if n.size > 0:
                            new_nbrs.append(n - refPos_nbr)
                        else:
                            new_nbrs.append(n)
                    
                    augmented_samples.append((new_hist, new_fut, new_nbrs, lat_enc, lon_enc))
        ...
        mask_batch = torch.zeros(len(samples), self.grid_size[1], self.grid_size[0], self.enc_size).byte()
        ...
        for sampleId,(hist, fut, nbrs, lat_enc, lon_enc) in enumerate(samples):
            ...
            if fut.size > 0:
                f_len = min(len(fut), self.t_f//self.d_s)
                fut_batch[0:f_len, sampleId, 0] = torch.from_numpy(fut[0:f_len, 0])
                fut_batch[0:f_len, sampleId, 1] = torch.from_numpy(fut[0:f_len, 1])
                op_mask_batch[0:f_len, sampleId, :] = 1
            ...
            for id, nbr in enumerate(nbrs):
                if len(nbr) != 0:
                    ...
                    pos_grid = [id % self.grid_size[0], id // self.grid_size[0]]
                    mask_batch[sampleId, pos_grid[1], pos_grid[0], :] = 1 
                    count += 1
